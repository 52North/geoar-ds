...
public class HeightMap extends RenderDetails {
	...
	public HeightMap() {
		try {
			...
			final int[] heightMapIndexData = new int[(verticesPerStrip * numStripsRequired)
					+ numDegensRequired];
			...
			for (int y = 0; y < yLength - 1; y++) {
				if (y > 0) {
					...
					heightMapIndexData[offset++] = (int) (y * yLength);
				}
				...
				if (y < yLength - 2) {
					...
					heightMapIndexData[offset++] = (int) (((y + 1) * yLength) + (xLength - 1));
				}
			}

			indexCount = heightMapIndexData.length;
			this.drawingMode = GLES20.GL_TRIANGLE_STRIP;
			setRenderDetails(vertices, colors, normals, heightMapIndexData);
		} catch (Throwable t) {
			Log.w(TAG, t);
		}
	}
